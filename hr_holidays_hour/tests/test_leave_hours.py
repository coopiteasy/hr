...
class TestLeaveHours(common.TransactionCase):
    def setUp(self):
        ...
        self.calendar = Calendar.create({"name": "Calendar 1"})
        ...
        self.employee_1 = Employee.create(
            {"name": "Employee 1", "calendar_id": self.calendar.id}
        )
        self.employee_2 = Employee.create(
            {"name": "Employee 2", "calendar_id": self.calendar.id}
        )
        self.employee_3 = Employee.create({"name": "Employee 3"})
        self.employee_4 = Employee.create(
            {"name": "Failing Employee", "calendar_id": self.calendar.id}
        )
        ...
        self.status_1 = HolidaysStatus.create(
            {"name": "Status 1", "limit": True}
        )
        self.status_2 = HolidaysStatus.create(
            {"name": "Status 2", "limit": False}
        )
    ...
    def test_05_get_working_intervals_of_day(self):
        interval = self.calendar.get_working_intervals_of_day(
            self.holiday_start, self.holiday_end
        )

        self.assertEqual(interval[0], [(self.holiday_start, self.holiday_end)])
    ...
    def test_07_get_hours(self):
        ...
        self.assertEqual(
            self.status_1.name_get(), [(self.status_1.id, "Status 1")]
        )
